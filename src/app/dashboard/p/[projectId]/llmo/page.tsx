...
import { MessageSquare, AlertCircle, Sparkles, TrendingUp, TrendingDown, Bot, CheckCircle2, ShieldAlert, Clock } from 'lucide-react'
import type { AeoEngine } from '@/generated/prisma'
import { cn } from '@/lib/utils'

export const metadata: Metadata = { title: 'LLMO' }

type Props = { params: Promise<{ projectId: string }> }

export default async function LlmoPage({ params }: Props) {
  ...
  const [prompts, gaps, pendingApprovals, generatedThisWeek, approvedThisWeek, lastSnapshot, thisWeekCited, lastWeekCited, prevWeekGenerated, prevWeekApproved] = await Promise.all([
    listPrompts(ctx.tenant.id, undefined, projectId),
    detectCitationGaps(ctx.tenant.id, ctx.project.ownDomain, projectId),
    prisma.approvalItem.count({
      where: { tenantId: ctx.tenant.id, module: 'aeo', status: 'PENDING', projectId },
    }),
    prisma.approvalItem.count({
      where: { tenantId: ctx.tenant.id, module: 'aeo', createdAt: { gte: weekAgo }, projectId },
    }),
    prisma.approvalItem.count({
      where: { tenantId: ctx.tenant.id, module: 'aeo', status: 'APPROVED', reviewedAt: { gte: weekAgo }, projectId },
    }),
    ...
    prisma.aeoRankSnapshot.groupBy({
      by: ['promptId'],
      where: { tenantId: ctx.tenant.id, projectId, snapshotDate: { gte: weekAgo }, ownRank: { not: null } },
    }).then((r) => r.length),
    prisma.aeoRankSnapshot.groupBy({
      by: ['promptId'],
      where: { tenantId: ctx.tenant.id, projectId, snapshotDate: { gte: twoWeeksAgo, lt: weekAgo }, ownRank: { not: null } },
    }).then((r) => r.length),
    prisma.approvalItem.count({
      where: { tenantId: ctx.tenant.id, module: 'aeo', createdAt: { gte: twoWeeksAgo, lt: weekAgo }, projectId },
    }),
    prisma.approvalItem.count({
      where: { tenantId: ctx.tenant.id, module: 'aeo', status: 'APPROVED', reviewedAt: { gte: twoWeeksAgo, lt: weekAgo }, projectId },
    }),
  ])
  ...
  const citationRate = activePrompts > 0 ? Math.round((citedCount / activePrompts) * 100) : 0
  const citationTrend = lastWeekCited > 0
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">LLMO ダッシュボード</h1>
          {lastSnapshot && (
            <p className={cn('mt-0.5 text-xs', snapshotStale ? 'text-amber-600 dark:text-amber-400' : 'text-muted-foreground')}>
          ...
          <Link key={stat.label} href={stat.href} className="group">
            <Card className="transition-all hover:shadow-md hover:border-primary/30">
              <CardContent className="pt-5 pb-4">
                ...
                {stat.sub && <p className={cn('mt-0.5 text-xs font-medium', stat.iconColor)}>{stat.sub}</p>}
                {(() => {
                  const t = 'trend' in stat ? stat.trend : undefined
                  if (t == null) return null
                  return (
                    <p className={cn('mt-0.5 text-xs font-medium', t >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-destructive')}>
                      {t >= 0 ? `+${t}%` : `${t}%`} 先週比
                    </p>
                  )
                })()}
      ...
      {uncitedWithoutPendingSuggestion.length > 0 && pendingApprovals === 0 && (
        <div className="mt-5 flex items-center justify-between rounded-lg border border-primary/20 bg-primary/5 px-4 py-3 text-sm">
          <div>
            <p className="font-medium text-foreground">{uncitedWithoutPendingSuggestion.length} 件のプロンプトが未引用です</p>
            <p className="mt-0.5 text-xs text-muted-foreground">AI 改善提案を一括生成してコンテンツを最適化できます</p>
          </div>
          <Link
            href={`${base}/prompts`}
      ...
      {(generatedThisWeek > 0 || approvedThisWeek > 0 || gaps.length > 0) && (
        <div className="mt-6">
          <h2 className="mb-3 text-xs font-semibold uppercase tracking-wide text-muted-foreground">今週の活動</h2>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[
              { label: 'AI 改善提案生成', value: generatedThisWeek, delta: weekDelta(generatedThisWeek, prevWeekGenerated), Icon: Sparkles, href: `${base}/suggestions`, color: 'text-primary' },
              { label: '提案承認', value: approvedThisWeek, delta: weekDelta(approvedThisWeek, prevWeekApproved), Icon: CheckCircle2, href: `${base}/suggestions?status=APPROVED`, color: approvedThisWeek > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-muted-foreground' },
              { label: '引用ギャップ', value: gaps.length, delta: null, Icon: ShieldAlert, href: `${base}/gaps`, color: gaps.length > 0 ? 'text-destructive' : 'text-muted-foreground' },
              { label: '引用済みプロンプト', value: citedCount, delta: citationTrend, Icon: citedCount > 0 ? TrendingUp : TrendingDown, href: `${base}/prompts`, color: citedCount > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-muted-foreground' },
            ].map((item) => (
              <Link key={item.label} href={item.href}>
                <Card className="transition-colors hover:border-primary/30">
                  <CardContent className="pt-4 pb-3">
                    ...
                    {item.delta !== null && (
                      <p className={cn('mt-0.5 text-xs font-medium', item.delta >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-destructive')}>
      ...
      {engineStats.length > 0 && (
        <div className="mt-6">
          <h2 className="mb-3 text-xs font-semibold uppercase tracking-wide text-muted-foreground">AI 検索エンジン別 引用率</h2>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {engineStats.map((e) => {
                ...
                <Card key={e.engine}>
                  <CardContent className="pt-4 pb-3">
                    <div className="mb-2 flex items-center justify-between">
                      ...
                      <Bot className={cn('h-3.5 w-3.5', good ? 'text-emerald-500' : warn ? 'text-amber-500' : 'text-destructive')} />
                    </div>
                    <p className={cn('text-2xl font-bold tabular-nums', good ? 'text-emerald-600 dark:text-emerald-400' : warn ? 'text-amber-600 dark:text-amber-400' : 'text-destructive')}>
                      {e.rate !== null ? `${e.rate}%` : '-'}
                    </p>
                    <p className="mt-0.5 text-xs text-muted-foreground">{e.cited}/{e.total} プロンプト引用</p>
                    <div className="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-muted">
                      <div className={cn('h-full rounded-full transition-all', good ? 'bg-emerald-500' : warn ? 'bg-amber-500' : 'bg-destructive')} style={{ width: `${e.rate ?? 0}%` }} />
                    </div>
                  </CardContent>
                </Card>
              )
