...
import { listPrompts } from '@/modules/llmo'
import { prisma } from '@/lib/db/client'
import type { AeoEngine } from '@/generated/prisma'
import SyncLlmoButton from '@/app/dashboard/llmo/prompts/sync-llmo-button'
import EmptyState from '@/components/empty-state'
import { MessageSquare, BarChart2, AlertCircle } from 'lucide-react'
import PromptSuggestButton from '@/app/dashboard/llmo/prompts/prompt-suggest-button'
import DeletePromptButton from '@/app/dashboard/llmo/prompts/[promptId]/delete-prompt-button'
...
export default async function PromptsPage({ params, searchParams }: Props) {
  ...
  const [prompts, industryCounts, lastSnapshot] = await Promise.all([
    listPrompts(ctx.tenant.id, industry || undefined, projectId),
  ...
  const countByIndustry = Object.fromEntries(industryCounts.map((c) => [c.industry ?? '', c._count]))
    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <SyncLlmoButton projectId={projectId} />
          <Link
            href={`${base}/prompts/from-templates`}
      ...
      {syncStale && activePrompts.length > 0 && (
        <div className="mb-4 flex items-center gap-2 rounded-lg border border-amber-300/50 bg-amber-50 px-4 py-2.5 text-sm text-amber-800 dark:border-amber-700/40 dark:bg-amber-950/50 dark:text-amber-300">
          ...
          <span>LLMO データが <strong>{syncDaysAgo}日</strong> 更新されていません。「同期」ボタンで最新データを取得してください。</span>
        </div>
      )}
      ...
      {activePrompts.length > 0 && (
        <div className="mb-4 flex flex-wrap items-center gap-4 rounded-lg border border-border bg-muted/30 px-4 py-2.5 text-sm">
          ...
          {fullUncited.length > 0 && (
            <span className="text-amber-600 dark:text-amber-400">完全未引用: {fullUncited.length}件</span>
          )}
        </div>
      )}
      ...
      {fullUncited.length >= 2 && pendingCount === 0 && (
        <div className="mb-5 flex items-center justify-between rounded-lg border border-primary/20 bg-primary/5 px-4 py-3 text-sm">
          <div>
            <p className="font-medium text-foreground">{fullUncited.length} 件のプロンプトがすべてのエンジンで未引用です</p>
            <p className="mt-0.5 text-xs text-muted-foreground">各プロンプトの詳細ページから AI 改善提案を生成できます</p>
          </div>
          <Link
        ...
        <div className="mb-4 flex flex-wrap gap-1 border-b border-border">
          <Link
            ...
            className={[
              ...
              !industry ? 'border-primary text-foreground font-medium' : 'border-transparent text-muted-foreground hover:text-foreground',
            ].join(' ')}
          >
            すべて
            <span className={['rounded-full px-1.5 py-0.5 text-xs', !industry ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'].join(' ')}>
              {total}
            </span>
          </Link>
          {industries.map((ind) => {
        ...
        <EmptyState
          ...
          action={total === 0 ? (
            <div className="flex gap-2">
              <Link href={`${base}/prompts/from-templates`} className={buttonVariants({ variant: 'outline', size: 'sm' })}>
                テンプレートから追加
              </Link>
              <Link href={`${base}/prompts/new`} className={buttonVariants({ size: 'sm' })}>
                + 追加
              </Link>
            </div>
          ) : undefined}
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              {ENGINES.map((e) => <TableHead key={e}>{ENGINE_LABELS[e]}</TableHead>)}
          ...
          <TableBody>
            {prompts.map((p) => (
              <TableRow key={p.id}>
                <TableCell>
                  <Link href={`${base}/prompts/${p.id}`} className="font-medium hover:underline">
                    {p.text.length > 60 ? `${p.text.slice(0, 60)}…` : p.text}
                  </Link>
                  {p.industry && <span className="ml-2 text-xs text-muted-foreground">{p.industry}</span>}
                  {!p.isActive && <span className="ml-2 text-xs text-muted-foreground">(無効)</span>}
                </TableCell>
                {ENGINES.map((e) => {
                ...
                <TableCell className="text-xs text-muted-foreground">
                  {p.lastSyncedAt ? p.lastSyncedAt.toLocaleDateString('ja-JP') : '-'}
                </TableCell>
                <TableCell>
                  <div className="flex items-center gap-2">
                    {p.isActive && Object.values(p.citationsByEngine).every((r) => r === null) && (
                      <PromptSuggestButton promptId={p.id} projectId={projectId} />
                    )}
                    <DeletePromptButton
                      promptId={p.id}
                      projectId={projectId}
                      redirectTo={`${base}/prompts`}
                    />
                  </div>
                </TableCell>
              </TableRow>
