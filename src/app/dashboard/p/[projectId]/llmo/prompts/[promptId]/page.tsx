...
import { getPrompt, getSnapshotsForPrompt } from '@/modules/llmo'
import type { AeoEngine } from '@/generated/prisma'
import SuggestButton from '@/app/dashboard/llmo/prompts/[promptId]/suggest-button'
import CompetitorManager from '@/app/dashboard/llmo/prompts/[promptId]/competitor-manager'
import DeletePromptButton from '@/app/dashboard/llmo/prompts/[promptId]/delete-prompt-button'
import Sparkline from '@/components/sparkline'

type Props = { params: Promise<{ projectId: string; promptId: string }> }
...
export default async function PromptDetailPage({ params }: Props) {
  ...
  const [prompt, snapshots] = await Promise.all([
    getPrompt(ctx.tenant.id, promptId, projectId),
    getSnapshotsForPrompt(ctx.tenant.id, promptId, 30, projectId),
  ])
    ...
    <div className="space-y-8">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            ...
            {!prompt.isActive && <Badge variant="outline" className="ml-2">無効</Badge>}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <DeletePromptButton promptId={promptId} projectId={projectId} redirectTo={`${base}/prompts`} />
          <SuggestButton promptId={promptId} projectId={projectId} />
        </div>
      </div>
      ...
      {Object.keys(latestByEngine).length > 0 && (
        <div className={cn(
          ...
          <div className="flex flex-wrap items-center gap-4 text-sm">
            <div>
              <p className={cn('font-medium', citedEngines.length > 0 ? 'text-emerald-700 dark:text-emerald-400' : 'text-destructive')}>
                {citedEngines.length > 0
              ...
              {dates.map((d) => {
                ...
                return (
                  <TableRow key={d}>
                    ...
                    {(['CHATGPT', 'GEMINI', 'GOOGLE_AI_OVERVIEW'] as AeoEngine[]).map((e) => (
                      <TableCell key={e}>
                        {!(e in row) ? (
      ...
      <section>
        ...
        <CompetitorManager
          promptId={promptId}
          projectId={projectId}
          initialCompetitors={prompt.competitors.map((c) => ({ id: c.id, domain: c.domain }))}
        />
      </section>
    </div>
  )
