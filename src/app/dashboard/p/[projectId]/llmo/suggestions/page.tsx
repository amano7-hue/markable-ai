...
import { parseLlmoSuggestionPayload } from '@/modules/llmo'
import ApproveButton from '@/app/dashboard/llmo/suggestions/approve-button'
import EmptyState from '@/components/empty-state'
import { Lightbulb, Clock } from 'lucide-react'
import { cn } from '@/lib/utils'

export const metadata: Metadata = { title: '改善提案 — LLMO' }
...
export default async function SuggestionsPage({ params, searchParams }: Props) {
  ...
  const [items, filteredTotal, statusCounts] = await Promise.all([
    ...
    prisma.approvalItem.groupBy({
      by: ['status'],
      where: { tenantId: ctx.tenant.id, module: 'aeo', projectId },
      _count: true,
    }),
  ])
    ...
    <div>
      <div className="mb-5 flex items-center justify-between gap-4">
        ...
        {total > 0 && (
          <div className="flex items-center gap-4 text-sm">
            {approvalRate !== null && (
              <span className={cn('font-medium', approvalRate >= 70 ? 'text-emerald-600 dark:text-emerald-400' : approvalRate >= 40 ? 'text-amber-600 dark:text-amber-400' : 'text-destructive')}>
                承認率 {approvalRate}%
              </span>
            )}
      ...
      {items.length === 0 ? (
        ...
        <div className="space-y-4">
          ...
          {items.map((item) => {
            let payload: ReturnType<typeof parseLlmoSuggestionPayload> | null = null
            try {
              payload = parseLlmoSuggestionPayload(item.payload)
            } catch {
              // invalid payload
            }
            ...
            return (
              <Card key={item.id} className={cn(isStale && 'border-amber-300/60 dark:border-amber-700/40')}>
                <CardHeader className="pb-2">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 space-y-1">
                      <p className="text-sm text-muted-foreground">{payload?.promptText ?? '不明なプロンプト'}</p>
                      <div className="flex items-center gap-2">
                        <p className="text-xs text-muted-foreground">{item.createdAt.toLocaleDateString('ja-JP')}</p>
                        {isStale && (
                ...
                <CardContent className="space-y-4">
                  {payload && (
                    <div className="rounded-md bg-muted p-3 text-sm">{payload.suggestion}</div>
                  )}
                  {item.status === 'PENDING' && (
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between pt-2">
              <Link
                ...
                className={`text-sm px-3 py-1.5 rounded-md border ${page <= 1 ? 'pointer-events-none opacity-40 border-transparent' : 'border-border hover:bg-accent'}`}
              >
                ← 前のページ
              </Link>
              ...
              <Link
                ...
                className={`text-sm px-3 py-1.5 rounded-md border ${page >= totalPages ? 'pointer-events-none opacity-40 border-transparent' : 'border-border hover:bg-accent'}`}
              >
                次のページ →
              </Link>
            </div>
