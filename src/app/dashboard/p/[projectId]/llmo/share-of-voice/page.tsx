...
export default async function ShareOfVoicePage({ params }: Props) {
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
                ...
                {engines.map((e) => (
                  <th key={e} className="px-3 py-3 text-center font-medium text-muted-foreground text-xs">{ENGINE_LABELS[e]}</th>
                ))}
            ...
            <tbody>
              {domains.map((d) => (
                <tr key={d.domain} className={cn('border-b border-border last:border-0', d.isOwn && 'bg-primary/5')}>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <span className={cn('font-medium truncate max-w-[200px]', d.isOwn && 'text-primary')}>{d.domain}</span>
                  ...
                  {engines.map((e) => {
                    ...
                    return (
                      <td key={e} className="px-3 py-3 text-center">
                        <span className={cn('text-sm font-medium tabular-nums', pct >= 50 ? 'text-emerald-600 dark:text-emerald-400' : pct >= 20 ? 'text-amber-600 dark:text-amber-400' : 'text-muted-foreground')}>
                          {pct}%
                        </span>
                      </td>
                    )
                    ...
                    <td className="px-4 py-2 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <div className="h-1.5 w-16 overflow-hidden rounded-full bg-muted">
                          <div className={cn('h-full rounded-full', p.pct >= 50 ? 'bg-emerald-500' : p.pct >= 20 ? 'bg-amber-500' : 'bg-destructive')} style={{ width: `${p.pct}%` }} />
                        </div>
                        <span className={cn('text-sm font-medium tabular-nums w-10 text-right', p.pct >= 50 ? 'text-emerald-600 dark:text-emerald-400' : p.pct >= 20 ? 'text-amber-600 dark:text-amber-400' : 'text-destructive')}>
                          {p.pct}%
                        </span>
                      </div>
                    </td>
