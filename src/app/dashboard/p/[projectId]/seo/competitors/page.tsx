...
export default async function CompetitorsPage({ params }: { params: Promise<{ projectId: string }> }) {
  ...
  const snaps = await prisma.seoKeywordSnapshot.findMany({
    where: { tenantId, projectId, snapshotDate: latestSnap.snapshotDate },
    include: {
      keyword: {
        select: {
          id: true,
  ...
  for (const g of gaps) {
    ...
    if (!intentMap.has(key)) {
      intentMap.set(key, { intent: key, total: 0, noContent: 0, weakContent: 0, totalImpressions: 0, topPosition: 999 })
    }
  ...
  const intentSummaries = [...intentMap.values()].sort((a, b) => b.totalImpressions - a.totalImpressions)
                ...
                return (
                  <Card key={intentKey} className="overflow-hidden">
                    ...
                    <CardContent className="pb-4">
                      ...
                      {s.noContent > 0 && (
                        <Link
                          href={`/dashboard/p/${projectId}/seo/keywords?intent=${intentKey}`}
                    ...
                    <TableRow key={g.keywordId}>
                      <TableCell>
                        <div className="flex items-center gap-2">
                          ...
                          {g.intent && (
                            <Badge variant="outline" className={`text-xs ${INTENT_COLORS[g.intent] ?? ''}`}>
