...
export async function POST(req: Request) {
  ...
  if (parsed.data.action === 'structure') {
    const { keyword, title, additionalInstructions, language } = parsed.data

    const langNote = language === 'en'
      ? '\n- All heading text MUST be written in English'
      : ''
...
${additionalInstructions ? '- 上記の追加指示を最優先で反映すること' : ''}${langNote}
