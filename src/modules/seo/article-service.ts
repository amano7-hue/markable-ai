...
async function analyzeReaderNeeds(
  ...
  const langNote = language === 'en'
    ? '\n\nOutput language: English. Write all values (targetAudience, keyQuestions, painPoints) in English.'
    : ''
...
keyQuestions と painPoints はSERP実データを踏まえて深掘りした分析を記述してください。${langNote}`,
  })
...
async function generateHeadingStructure(
  ...
  const langNote = language === 'en'
    ? '\n\nAll heading text (h1, h2, h3s) MUST be written in English.'
    : ''
...
- 最後のセクションはまとめ・CTAを含める${relatedKeywords ? `\n\n【関連キーワード必須】以下のキーワードをH2またはH3に自然な形で必ず含めること:\n${relatedKeywords}` : ''}${avoidSensationalHeadings ? '\n\n【見出しトーン厳守】煽情的・あおり系の表現（「衝撃の〜」「絶対に〜すべき」「今すぐ〜しないと危険」「知らないと損」等）は一切使用しない。中立的・専門的・情報提供型の見出しにすること。' : ''}${langNote}`,
  })
  const text = result.text ?? ''
  const match = text.match(/\{[\s\S]*\}/)
...
async function generateSeoMeta(
  ...
  const langNote = language === 'en'
    ? '\n\nOutput language: English. Write seoTitle and seoDescription in English. Title limit: 60 characters. Description limit: 155 characters.'
    : ''
...
`
  ...
  const openingInstruction = language === 'en'
    ? `Write a B2B marketing SEO article in English following the conditions below. All headings, body text, lists, and tables MUST be written in English.`
    : `以下の条件でBtoBマーケティング向けSEO記事を日本語で執筆してください。`

  const constructedPrompt = `${openingInstruction}
...
# 出力フォーマット（厳守）
- 見出しは必ず <h1>〜<h5> タグを使用する（# ## ### の Markdown は使用禁止）。通常はH3まで、内容の階層化が必要な場合はH4・H5も使用可
- 本文の段落は <p> タグで囲む。<p>タグ内では <br> タグを使用しない（改行が必要なら新しい <p> タグで区切る）
- 箇条書きは <ul><li>〜</li></ul>、番号付きリストは <ol><li>〜</li></ol>
- **太字** や *斜体* などの Markdown 記法は使用禁止。太字は <strong>、斜体は <em>
- <strong>: 重要な専門用語・定義・核心的なポイント（各段落で1〜2箇所のみ）
- <mark class="highlight">: 読者に最も伝えたいインサイト・結論・数字（記事全体で5〜8箇所）
- <u>: 補足説明や注意喚起が必要なキーワード（記事全体で3〜5箇所）
- <em>: 強調したい語句・概念（記事全体で3〜5箇所）
...
export async function generateArticleDraft(
  ...
  if (input.precomputedReader && input.precomputedCompetitor) {
    ...
    const [rRaw, comp] = await Promise.all([
      analyzeReaderNeeds(input.title, keywordText, { relatedQuestions, relatedSearches, organicSnippets }, input.language ?? 'ja'),
  ...
  const articleLanguage: 'ja' | 'en' = input.language === 'en' ? 'en' : 'ja'

  // ─── Step 1b: タイトル最適化（上位記事を参考にSEOタイトルを生成） ──
  const optimizedTitle = organicResults.length > 0
    ? await optimizeTitleFromSerp(input.title, keywordText, organicResults, articleLanguage).catch(() => input.title)
    : input.title
  ...
  const seoMeta = await generateSeoMeta(effectiveTitle, keywordText, reader, headings, articleLanguage)
  ...
  await generateImagesForArticle(article.id, tenantId, effectiveTitle, keywordText, diagramSpecs, brandProfile, articleLanguage)
...
export async function regenerateArticle(
  ...
  const [diagramResult, tableResult] = await Promise.allSettled([
    generateDiagrams(
      optimizedDraft, existing.title, keywordText,
      brandProfile?.diagramPreference as string | null ?? null,
      brandProfile?.diagramInstructions as string | null ?? null,
    ),
    generateTables(optimizedDraft, existing.title, keywordText),
  ])
...
export function optimizeHtml(html: string): string {
  return html
    ...
    // 残った<p>内の単独<br>を</p><p>に変換（1行区切りを段落区切りに）
    .replace(/(<p[^>]*>[^<]*)<br\s*\/?>/gi, '$1</p><p>')
    // 空の段落を除去
    .replace(/<p[^>]*>\s*<\/p>/gi, '')
    // 連続する空白を整理
    .replace(/\n{3,}/g, '\n\n')
...
async function generateImagesForArticle(
  ...
  const imgTextLang = language === 'en'
    ? 'IMPORTANT: All text in this image MUST be written in English. Do NOT use Japanese.'
    : 'IMPORTANT: All text in this image MUST be written in Japanese (日本語). Do NOT use English.'
  const imgTitleInstruction = language === 'en'
    ? `Display the following title text prominently in the image: "${title}". Use the title text as-is, do not add dates or extra text.`
  ...
  for (const [i, spec] of diagramSpecs.entries()) {
    const diagramLangInstruction = language === 'en'
      ? 'IMPORTANT: All text labels in this image MUST be written in English. Do NOT use Japanese. Do not add dates, year, or copyright notices.'
      : 'IMPORTANT: All text in this image MUST be written in Japanese (日本語). Do NOT use English text. すべてのテキストラベルは日本語で表示すること。年号・日付・著作権表示は追加しないこと。'
    const diagramPrompt = [
      spec.imagePrompt,
...
[
  {
    "marker": "diag-1",
    "title": "${language === 'en' ? 'Diagram caption in English' : '図解のキャプション（日本語）'}",
    "mermaidCode": "graph TD\\n  A[\\"${language === 'en' ? 'Step 1' : 'ステップ1'}\\"] --> B[\\"${language === 'en' ? 'Step 2' : 'ステップ2'}\\"]",
    "imagePrompt": "Infographic showing the step-by-step process of [concept in English]. Include [specific elements] with ${language === 'en' ? 'English' : 'Japanese'} labels.",
    "insertAfterH2": "${language === 'en' ? 'H2 heading text to insert after (no tags)' : '挿入するH2の見出しテキスト（タグなし）'}"
  }
]`,
    })
