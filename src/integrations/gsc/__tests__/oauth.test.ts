...
import { getGscAuthUrl, exchangeCodeForTokens, refreshAccessToken } from '../oauth'
...
describe('getGscAuthUrl', () => {
  it('returns a Google OAuth URL', () => {
    const url = getGscAuthUrl('tenant-state')
    expect(url).toContain('accounts.google.com/o/oauth2/v2/auth')
  })

  it('includes the state parameter', () => {
    const url = getGscAuthUrl('my-tenant-id')
    expect(url).toContain('state=my-tenant-id')
  })

  it('includes GSC webmasters scope', () => {
    const url = getGscAuthUrl('t1')
    expect(url).toContain('webmasters.readonly')
  })

  it('includes offline access_type', () => {
    const url = getGscAuthUrl('t1')
...
describe('refreshAccessToken', () => {
  it('returns new accessToken', async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: () => Promise.resolve({ access_token: 'new-tok', expires_in: 3600 }),
    })
